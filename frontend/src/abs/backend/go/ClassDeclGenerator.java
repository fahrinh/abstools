...
public class ClassDeclGenerator {
    ...
    public ClassDeclGenerator(PrintStream stream, ClassDecl decl) {
        this.stream = stream;
        this.decl = decl;
        className = GoBackend.getClassName(decl.getName());
    }
...
//        generateConstructor(); TODO needed in Go ?
...
//        GoGeneratorHelper.generateParams(stream, decl.getParams());
//        stream.print(" { return (T)");
//        stream.print(className + ".__ABS_createNewObject");
//        GoGeneratorHelper.generateParamArgs(stream, "null", decl.getParams());
//        stream.println("; }");
//
//        // static constructor method for new C
//        stream.print("public static final <T extends " + className + "> T __ABS_createNewObject");
//        GoGeneratorHelper.generateParams(stream, ABSObject.class.getName() + " __ABS_source", decl.getParams());
//        stream.println(" {");
...
//        stream.println("}");
    ...
    private void generateObjectConstruction(String runtime) {
        ...
        if (decl.isForeign()) {
            ...
            GoGeneratorHelper.generateParamArgs(stream,"\"" + decl.getModuleDecl().getName() + "." + decl.getName() + "\"", decl.getParams());
            stream.println(";");
            stream.print("if (__ABS_result == null) __ABS_result = ");
        }

        stream.print("new " + className);
        GoGeneratorHelper.generateParamArgs(stream,decl.getParams());
        stream.println(";");
...
//            stream.println("if (\"" + p.getName() + "\".equals(__ABS_fieldName)) return " + GoBackend.getVariableName(p.getName()) + ";");
//        }
//
//        for (FieldDecl f : decl.getFields()) {
//            stream.println("if (\"" + f.getName() + "\".equals(__ABS_fieldName)) return " + GoBackend.getVariableName(f.getName()) + ";");
//        }
...
//        stream.println("}");
    ...
    private void generateConstructor() {
        ...
        for (ParamDecl p : decl.getParams()) {
            stream.println("this." + GoBackend.getVariableName(p.getName()) + " = " + GoBackend.getVariableName(p.getName()) + ";");
        }
        ...
        for (FieldDecl f : decl.getFields()) {
            if (f.hasInitExp()) {
                stream.print("this.");
                stream.print(GoBackend.getVariableName(f.getName()));
                stream.print(" = ");
